    <section class="produto_item">
        <div class="wrapper">
            <a href="?p=chocolate">Voltar</a>
            <a href="?p=leitecond">Próximo</a>
        </div>
        <div class="produto_item_container">
            <img src="images/p3.png" alt="Cocada de Maracujá">
            <div class="produto_item_descricao">
                <h1>Cocada de Maracujá</h1>
                <h5>Linha Frutas</h5>
                <h2>Ingredientes</h2>
                <p>Coco ralado, leite condensado, açúcar, margarina e maracujá.</p>
                <h2>Peso</h2>
                <p>300mg</p>
                <h2>Preço: <strong>R$ 7,90</strong></h2>
                <a href="?p=contato">Eu quero!</a>
            </div>
        </div>
    </section>